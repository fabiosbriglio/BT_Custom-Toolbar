...
for sheet in sheets:
    for v_id in sheet.GetAllPlacedViews():  # Get view IDs placed on the sheet
        views_on_sheets.add(v_id.IntegerValue)  # Store as an integer for comparison

# Debugging Output
output = script.get_output()
output.print_md("### Checking Views Placed on Sheets (to be deleted)")

# List of views to delete (ONLY those placed on sheets)
views_to_delete = []
for v in views:
    try:
        # Only delete views that ARE placed on a sheet
        if v.Id.IntegerValue in views_on_sheets:
            views_to_delete.append(v)
            output.print_md("🗑 Marking for Deletion: {} (Placed on Sheet)".format(v.Name))
        else:
            output.print_md("✅ Keeping: {} (Not on Sheet)".format(v.Name))
...
if not views_to_delete:
    forms.alert("No views found on sheets! Nothing to delete.", exitscript=True)
    ...
    confirm = forms.alert(
        ...
        msg="The following {} views are placed on sheets and will be deleted:\n\n{}\n\nContinue?".format(len(views_to_delete), view_names),
        ok=True, cancel=True
    )
    ...
    if confirm:
        try:
            # Start a transaction
            t = Transaction(doc, "Delete Views Placed on Sheets")
            t.Start()
            ...
            # Show result
            forms.alert("✅ Deleted {} views that were placed on sheets!".format(deleted_count))
